...
class PurchaseReturnRepository extends AbstractValidator implements PurchaseReturnInterface {
	...
	private function setPurchaseReturnLog($attributes, $key, $document_id)
	{
		...
		if($attributes['quantity'][$key]==$attributes['actual_quantity'][$key]) {								 
			DB::table('item_stock')
					->where('document_id', $attributes['purchase_invoice_id'])->where('document_type', 'PI')->where('department_id', auth()->user()->department_id ?? 1)->where('item_id', $attributes['item_id'][$key])
					->update(['is_return' => 1]);
		}
	...
	{
		if($attributes['quantity'][$key]==$attributes['actual_quantity'][$key]) {
			$row = DB::table('item_stock')->where('item_id', $attributes['item_id'][$key])->where('department_id', auth()->user()->department_id ?? 1)
	...
	private function setInputValue($attributes)
	{
		...
		$this->purchase_return->department_id  =auth()->user()->department_id ?? 1;
	...
	{
		$prev = DB::table('item_stock')->where('document_type', 'PI')->where('department_id', auth()->user()->department_id ?? 1)->where('document_id', $attributes['purchase_invoice_id'])
						...
						if(isset($attributes['locqty'][$key])) {
							foreach($attributes['locqty'][$key] as $lk => $lq) {
								...
								if($lq!='') {
									...
									$itemLocationPR = new ItemLocationPR();
									$itemLocationPR->location_id = $attributes['locid'][$key][$lk];
									$itemLocationPR->department_id=auth()->user()->department_id ?? 1;
									$itemLocationPR->item_id = $value;
									$itemLocationPR->unit_id = $attributes['unit_id'][$key];
		...
		try {
			...
			if($attributes['remove_item']!='')
			{
				...
				foreach($arrids as $row) {
					...
					DB::table('item_location')->where('item_id', $res->item_id)->where('department_id',auth()->user()->department_id ?? 1)
								->where('location_id', $attributes['location_id'])
	...
	public function delete($id)
	{
		...
		try {
			...
			//Transaction update....
			DB::table('account_transaction')->where('voucher_type', 'PR')->where('department_id',auth()->user()->department_id ?? 1)->where('voucher_type_id',$id)->update(['status' => 0,'deleted_at' => date('Y-m-d H:i:s'),'deleted_by' => Auth::User()->id ]);
	...
	public function getItems($id)
	{
		...
		return $query->join('purchase_return_item AS poi', function($join) {
					  ...
					  ->where('purchase_return.department_id',auth()->user()->department_id ?? 1)
...
public function getReport($attributes)
{
	...
	private function setSaleLog($attributes, $key, $document_id, $cost_avg, $sale_cost, $action)
	{
		...
		$irow = DB::table('itemstock_department')->where('itemmaster_id', $attributes['item_id'][$key])->where('department_id',auth()->user()->department_id ?? 1)
	...
	public function updateLastPurchaseCostAndCostAvgonDelete($items, $id) {
		...
		foreach($items as $item) {
			...
			DB::table('item_log')->where('document_id', $id)->where('department_id', auth()->user()->department_id ?? 1)->where('document_type','PR')
			...
			DB::table('itemstock_department')->where('itemmaster_id', $item->item_id)->where('department_id', auth()->user()->department_id ?? 1)
	...
	{
		$query = $this->purchase_return->where('purchase_return.status',1)->where('purchase_return.department_id',auth()->user()->department_id ?? 1);
		return $query->join('account_master AS am', function($join) {
							$join->on('am.id','=','purchase_return.supplier_id');
						} )
					->count();

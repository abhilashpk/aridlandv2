...
class PurchaseOrderRepository extends AbstractValidator implements PurchaseOrderInterface {
	...
	private function setInputValue($attributes)
	{
		...
		$this->purchase_order->reference_no = isset($attributes['reference_no'])?$attributes['reference_no']:'';
		$this->purchase_order->description = isset($attributes['description'])?$attributes['description']:'';
		$this->purchase_order->terms_id = isset($attributes['terms_id'])?$attributes['terms_id']:'';
		$this->purchase_order->job_id = isset($attributes['job_id'])?$attributes['job_id']:'';
	...
	private function setItemInputValue($attributes, $purchaseOrderItem, $key, $value,$other_cost, $lineTotal, $total_quantity=null) 
	{
		...
		$purchaseOrderItem->quantity = (float)$attributes['quantity'][$key];
		...
		$purchaseOrderItem->discount = (float)$attributes['line_discount'][$key];
		...
		$purchaseOrderItem->vat_amount_fc = (isset($attributes['is_fc']))?round(($tax_total /$currencyrate),2):(float)$tax_total;
		$purchaseOrderItem->total_price_fc = (isset($attributes['is_fc']))?round(($line_total /  $currencyrate),2):(float)$line_total;
		$purchaseOrderItem->item_total_fc = (isset($attributes['is_fc']))?round(($item_total /  $currencyrate),2):(float)$item_total;

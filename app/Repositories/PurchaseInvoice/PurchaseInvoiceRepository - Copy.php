...
class PurchaseInvoiceRepository extends AbstractValidator implements PurchaseInvoiceInterface {
	...
	private function setInputValue($attributes)
	{
		...
		$this->purchase_invoice->department_id   =auth()->user()->department_id ?? 1; //isset($attributes['department_id'])?$attributes['department_id']:'';
	...
	private function updateLastPurchaseCostAndCostAvgonDelete($items, $id) {
		...
		foreach($items as $item) {
			...
			//COST AVG Updating on DELETE section....
			DB::table('item_log')->where('document_id', $id)->where('department_id',auth()->user()->department_id ?? 1)->where('document_type','PI')->where('quantity', $item->quantity)
			...
			DB::table('itemstock_department')->where('department_id',auth()->user()->department_id ?? 1)->where('itemmaster_id', $item->item_id)
	...
	private function setPurchaseLog($attributes, $key, $document_id, $cost_avg, $action,$other_cost, $item=null)
	{
		...
		if($action=='add') {
			...
			//MAY25
		    $slog = DB::table('item_log')->where('document_type','PI')->where('department_id',auth()->user()->department_id ?? 1)->where('document_id', $document_id)->where('item_id', $item->item_id)->where('unit_id', $item->unit_id)->where('item_row_id', $attributes['order_item_id'][$key])
		                ->select('id')->first();
			$logid = $slog->id;
	...
	private function updateItemQuantity($attributes, $key)
	{
		...
		$items = DB::table('itemstock_department')->where('department_id',auth()->user()->department_id ?? 1)
		                              ->where('itemmaster_id', $attributes['item_id'][$key])
									  ->where('is_baseqty', 1)->first();
		...
		if($items) {
			...
			$baseqty = ($qty * $attributes['packing'][$key]);
			DB::table('itemstock_department')
				->where('id', $items->id)->where('department_id',auth()->user()->department_id ?? 1)
			...
			if($amount_type=='VAT' || $amount_type=='VATOC') {
					...
					if($attributes['tax_code_old'][0]!=$attributes['tax_code'][0]) {
						...
						if( ($attributes['tax_code_old'][0]=='ZR' && $attributes['tax_code'][0]=='SR') || ($attributes['tax_code_old'][0]=='EX' && $attributes['tax_code'][0]=='SR') ) {
							...
							DB::table('account_transaction')
								...
								->where('voucher_type', 'PI')					
								->where('tr_for', 0)
									->update(['account_master_id' => $vatrow->collection_account, 'department_id' => auth()->user()->department_id ?? 1]);
				...
				if($attributes['account_master_id'] != $attributes['old_account_master_id']) {
					DB::table('account_transaction')
							...
							->update( ['account_master_id' => $attributes['account_master_id'], 'department_id' => auth()->user()->department_id ?? 1 ]);
		...
		if($this->isValid($attributes)) {
		 ...
		 try {
			...
			//VOUCHER NO LOGIC.....................
				...
				$qry = DB::table('purchase_invoice')->where('deleted_at', '0000-00-00 00:0:00')->where('status', 1)->where('department_id',auth()->user()->department_id ?? 1);
						...
						foreach($attributes['locqty'][$key] as $lk => $lq) {
							...
							if($lq!='') {
								...
								if($qtys) { //NOV24
									...
									//echo '<pre>';print_r($locid[$key][$lk]);exit;
		...
		try {
					...
					if(isset($attributes['locqty'][$key])) {
						foreach($attributes['locqty'][$key] as $lk => $lq) {
							if($lq!='') {
								...
								$edit = DB::table('item_location_pi')->where('department_id', auth()->user()->department_id ?? 1)->where('id', $attributes['editid'][$key][$lk])->where('is_sdo',0)->first();//NOV24
								...
								if($edit) {
									...
									$sdolog = DB::table('item_location_pi')->where('department_id', auth()->user()->department_id ?? 1)->where('item_id',$value)->where('unit_id',$attributes['unit_id'][$key])->where('invoice_id', $attributes['order_item_id'][$key])->where('is_sdo',0)->first();
								...
								//NOV24
								DB::table('item_location_pi')->where('id', $attributes['editid'][$key][$lk])->where('department_id', auth()->user()->department_id ?? 1)->update(['quantity' => $lcqty,'status' => 1, 'deleted_at' => '0000-00-00 00:00:00','qty_entry' => $lq]);
								

							} else { //NOV24
								DB::table('item_location_pi')->where('department_id', auth()->user()->department_id ?? 1)->where('id', $attributes['editid'][$key][$lk])->update(['quantity' => isset($lcqty)?$lcqty:'','status' => 0, 'deleted_at' => date('Y-m-d h:i:s'),'qty_entry' => $lq]);
							}
						}
					}
			...
			if($attributes['remove_item']!='')
			{
				...
				foreach($arrids as $row) {
					...
					$pirow = DB::table('item_location_pi')->where('department_id', auth()->user()->department_id ?? 1)->where('invoice_id',$row)->where('is_sdo',0)->get();
					foreach($pirow as $prow) {
						...
						DB::table('item_location')->where('location_id', $prow->location_id)>where('department_id', auth()->user()->department_id ?? 1)->where('item_id',$prow->item_id)->where('unit_id',$prow->unit_id)
			...
			if($this->setInputValue($attributes)) {
				...
				//if($this->purchase_invoice->voucher_date != date('Y-m-d', strtotime($attributes['voucher_date']))) {
					...
					DB::table('item_log')->where('document_type','PI')->where('department_id',auth()->user()->department_id ?? 1)->where('document_id',$this->purchase_invoice->id)
			...
			if(isset($attributes['remove_oc']) && $attributes['remove_oc']!='')
			{
				...
				foreach($arrids as $row) {
					...
					DB::table('account_transaction')->where('department_id',auth()->user()->department_id ?? 1)->where('voucher_type', 'PI')->where('voucher_type_id', $this->purchase_invoice->id)
		...
		try {
			...
			if($this->purchase_invoice->document_id > 0) {
				if($this->purchase_invoice->document_type=='PO') {
					...
					DB::table('item_log')->where('document_type','SDO')->where('department_id',auth()->user()->department_id ?? 1)->whereIn('document_id',$ids)->update(['status' => 1,'deleted_at' => '0000-00-00 00:00:00']);
					...
					foreach($doitems as $doitem) {
						...
						DB::table('item_location_pi')->where('department_id', auth()->user()->department_id ?? 1)->where('invoice_id',$doitem->id)->where('is_sdo',1)->update(['status'=> 1,'deleted_at'=> '0000-00-00 00:00:00']);
					}
					
					foreach($ids as $idd) {
				        $dorow = DB::table('item_log')->join('supplier_do_item','supplier_do_item.id','=','item_log.item_row_id')
				                    ->where('item_log.document_type','SDO')->where('item_log.department_id',auth()->user()->department_id ?? 1)->where('item_log.document_id',$idd)
				                    ->select('item_log.quantity','supplier_do_item.balance_quantity')->first();//supplier_do_item.quantity
				        if($dorow)
				            DB::table('item_log')->where('document_type','SDO')->where('department_id',auth()->user()->department_id ?? 1)->where('document_id',$idd)->update(['quantity' => $dorow->quantity, 'status' => 1,'deleted_at' => '0000-00-00 00:00:00']); //($dorow->balance_quantity > 0)?$dorow->balance_quantity:
				    }
			...
			//Transaction update....
			DB::table('account_transaction')->where('voucher_type', 'PI')->where('department_id',auth()->user()->department_id ?? 1)->where('voucher_type_id',$id)->update(['status' => 0,'deleted_at' => date('Y-m-d H:i:s'),'deleted_by' => Auth::User()->id ]);
			...
			foreach($items as $item) {
				...
				$pirow = DB::table('item_location_pi')>where('department_id', auth()->user()->department_id ?? 1)->where('invoice_id',$item->id)->where('is_sdo',0)->get();
				
				foreach($pirow as $prow) {
					DB::table('item_location_pi')->where('id',$prow->id)>where('department_id', auth()->user()->department_id ?? 1)->update(['status'=>0,'deleted_at'=>date('Y-m-d H:i:s')]);
	...
	{
		$query = $this->purchase_invoice->where('purchase_invoice.status',1)->where('purchase_invoice.department_id',auth()->user()->department_id ?? 1);
	...
	public function getSDOitems($id)
	{
		...
		return $query->join('purchase_invoice_item AS poi', function($join) {
					...
					->where('purchase_invoice.department_id',auth()->user()->department_id ?? 1)
					  ->where('poi.status',1)
	...
	{
		$query = $this->purchase_invoice->where('purchase_invoice.id', $id)->where('purchase_invoice.department_id',auth()->user()->department_id ?? 1);
		return $query->join('account_master AS am', function($join) {
							$join->on('am.id','=','purchase_invoice.supplier_id');
						} )
	...
	public function getItems($id)
	{
		...
		return $query->join('purchase_invoice_item AS poi', function($join) {
					  ...
					  ->where('poi.deleted_at','0000-00-00 00:00:00')
					  ->where('isd.department_id',auth()->user()->department_id ?? 1)
					  ->select('poi.*','u.unit_name','im.item_code','isd.is_baseqty','isd.packing','isd.pkno','ci.balance_quantity as so_balance_quantity')
					  ->orderBY('poi.id')
					  ->groupBY('poi.id')
					  ->get();
	...
	public function getItemLocation($id) {
		...
		return DB::table('item_location_pi')->where('department_id', auth()->user()->department_id ?? 1)->where('invoice_id', $id)->where('status',1)->where('deleted_at','0000-00-00 00:00:00')->get();
	}
	...
	public function getPurchaseIitems($id, $attributes)
	{
		...
		$date_from = ($attributes['date_from']!='')?date('Y-m-d', strtotime($attributes['date_from'])):'';
		$date_to = ($attributes['date_to']!='')?date('Y-m-d', strtotime($attributes['date_to'])):'';
		$query = $this->purchase_invoice->where('purchase_invoice.supplier_id',$id)->where('purchase_invoice.department_id',auth()->user()->department_id ?? 1);
		...
		return $query->select('poi.*','u.unit_name','im.item_code','isd.packing','isd.is_baseqty','poi.item_total AS line_total',
	...
	private function updateLastPurchaseCostCostAvgOtherCostOnTransfer($attributes, $key, $other_cost, $SDOlogs)
	{
		...
		if($SDOlogs->quantity==$quantity) {
	    	...
	    	DB::table('item_log')->where('document_type','SDO')->where('department_id',auth()->user()->department_id ?? 1)->whereIn('document_id',$pids)->update(['status'=> 0,'deleted_at'=>date('Y-m-d H:i:s')]);
		} else {
		    //PARTIAL TRANSFER OF DO TO SI HANDLING LOG PARIAL QUANTITY.....
		    $siquantity = $quantity;
		    foreach($pids as $pid) {
		       $drow = DB::table('item_log')->where('document_type','SDO')->where('department_id',auth()->user()->department_id ?? 1)->where('document_id',$pid)->select('quantity')->first();//25 < 15
		       if($drow && $drow->quantity <= $siquantity) {
		           DB::table('item_log')->where('document_type','SDO')->where('department_id',auth()->user()->department_id ?? 1)->where('document_id',$pid)->update(['status'=> 0,'deleted_at'=>date('Y-m-d H:i:s')]);
		           $siquantity = $siquantity - $drow->quantity;//15
		       } else {
		           $finalqty = $drow->quantity - $siquantity;
		           DB::table('item_log')->where('document_type','SDO')->where('department_id',auth()->user()->department_id ?? 1)->where('document_id',$pid)->update(['quantity'=> $finalqty]);
		       }
		    }
	...
	private function updateLastPurchaseCostCostAvgOtherCostOnTransferUpdate($attributes, $key, $other_cost, $SDOlogs)
	{
		...
		if($SDOlogs->quantity==$attributes['quantity'][$key]) {
		    //DISABLE SDO TRANSACTION LOG.... 50,25,25   90
	    	DB::table('item_log')->where('document_type','SDO')->where('department_id',auth()->user()->department_id ?? 1)->whereIn('document_id',$pids)->update(['quantity' => 0, 'status'=> 0,'deleted_at'=>date('Y-m-d H:i:s')]);
		} else {
		    //PARTIAL TRANSFER OF DO TO SI HANDLING LOG PARIAL QUANTITY.....
		    ...
		    foreach($pids as $pid) {
		       ...
		       if($drow && $drow->quantity < $siquantity) {
		           ...
		           if($finalqty==0)
		                DB::table('item_log')->where('document_type','SDO')->where('department_id',auth()->user()->department_id ?? 1)->where('document_id',$pid)->update(['quantity' => 0, 'status'=> 0,'deleted_at'=>date('Y-m-d H:i:s')]);
		           else
		                DB::table('item_log')->where('document_type','SDO')->where('department_id',auth()->user()->department_id ?? 1)->where('document_id',$pid)->update(['quantity'=> $finalqty, 'status'=> 1, 'deleted_at'=> '0000-00-00 00:00:00']);
		                
		           $siquantity = $siquantity - $drow->quantity;//15
		       } else {
		           $finalqty = $siquantity_act - $siquantity;  //echo $finalqty.' b';
		           DB::table('item_log')->where('document_type','SDO')->where('department_id',auth()->user()->department_id ?? 1)->where('document_id',$pid)->update(['quantity'=> $finalqty, 'status'=> 1, 'deleted_at'=>'0000-00-00 00:0:00']);
		       }
	...
	{	
		...
		foreach($itmlogs as $log) {
			...
			DB::table('item_log')->where('id',$log->id)->where('department_id',auth()->user()->department_id ?? 1)
						->update([
							 'cost_avg' => $cost_avg,
							 'pur_cost' => (isset($attributes['is_fc']))?($attributes['cost'][$key]*$attributes['currency_rate'])+$other_cost:$attributes['cost'][$key]+$other_cost,

...
class GoodsReturnController extends Controller
{
	...
	{
		\Log::info('Goods Return getPrint START', ['id' => $id, 'rid' => $rid]);

		// =====================================
		// 1️⃣ STRICT Document Department Check
		// =====================================
		$docDept = DB::table('goods_return')
						->where('id', $id)
						->value('department_id');
		...
		if (!$viewfile || empty($viewfile->print_name)) {
			...
			$titles = [
				'main_head' => 'Goods Return',
				'subhead'   => 'Goods Return'
			];
		...
		if (env('STIMULSOFT_VER') == 2) {
			...
			return view('body.goodsreturn.viewer')
						->withPath($path)
						->withView($viewfile->print_name)
						->with('id', $id);
		}

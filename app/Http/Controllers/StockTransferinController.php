...
class StockTransferinController extends Controller
{
   ...
   protected $itemmaster;
	...
	{
		\Log::info('Stock Transfer IN getPrint START', ['id' => $id, 'rid' => $rid]);

		// =====================================
		// 1️⃣ STRICT Document Department Check
		// =====================================
		$docDept = DB::table('stock_transferin')
						->where('id', $id)
						->value('department_id');
		...
		if (!$viewfile || empty($viewfile->print_name)) {
			...
			$titles = [
				'main_head' => 'Stock Transfer In',
				'subhead'   => 'Stock Transfer In'
			];
		...
		if (env('STIMULSOFT_VER') == 2) {
			...
			return view('body.stocktransferin.viewer')
						->withPath($path)
						->withView($viewfile->print_name)
						->with('id', $id);
		}
